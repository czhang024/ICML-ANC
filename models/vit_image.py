...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, global_pool=False, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        ######## Adapter begins #########   #Should be VPT Begins
    ...
    def forward_features(self, x):
        ...
        for idx, blk in enumerate(self.blocks):
            if self.tuning_config.vpt_on:    #ZC: add VPT modules
                eee = self.embeddings[idx].expand(B, -1, -1)   #ZC: expand vpt to all bs
                x = torch.cat([eee, x], dim=1)  # ZC: concatenate vpt into features/images
            x = blk(x)
            if self.tuning_config.vpt_on:
                x = x[:, self.tuning_config.vpt_num:, :] #ZC: remove vpt features and only keep x. This allows the dimension unchanged
